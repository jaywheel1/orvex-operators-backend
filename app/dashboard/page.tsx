...
// Task Card Component
function TaskCard({ task, onStart }: { task: TaskData; onStart: (task: TaskData) => void }) {
  const isLocked = task.status === 'coming_soon' || task.status === 'locked';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h4 className={`font-medium ${isLocked ? 'text-[#7d85d0]/70' : isCompleted ? 'text-[#b9f0d7]' : 'text-white'}`}>
              {task.name}
            </h4>
            {isCompleted && (
        ...
        <div className="text-right ml-3">
          <div className={`text-lg font-bold ${isLocked ? 'text-[#7d85d0]/50' : isCompleted ? 'text-[#b9f0d7]/60' : 'text-[#b9f0d7]'}`}>
            +{task.points}
          </div>
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#7d85d0]/10">
        <div className="flex items-center gap-2">
          ...
          {task.cap > 1 && (
            <span className="text-[10px] text-[#7d85d0]">
              {task.completions > 0 ? `${task.completions}/` : 'Max '}{task.cap}x
            </span>
          )}
        </div>
        {isLocked ? (
        ...
        ) : isCompleted && task.cap <= 1 ? (
          <span className="text-[10px] text-[#b9f0d7] flex items-center gap-1">
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
            Complete
...
function CategoryIcon({ categoryId, isLocked }: { categoryId: string; isLocked: boolean }) {
  const className = `w-6 h-6 ${isLocked ? 'text-[#7d85d0]/50' : 'text-[#6265fe]'}`;
  switch (categoryId) {
    case 'social':
    ...
    case 'trading':
      return <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>;
    case 'liquidity':
...
export default function DashboardPage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (data.ok) setUserData(data.data);
    } catch (err) {
  ...
  // Group tasks by category from API data
  const tasksByCategory = tasks.reduce<Record<string, TaskData[]>>((acc, task) => {
    if (!acc[task.category]) acc[task.category] = [];
    acc[task.category].push(task);
    return acc;
  }, {});

  const categoryMaxCp = (categoryId: string): number => {
    const catTasks = tasksByCategory[categoryId] || [];
    return catTasks.reduce((sum, t) => sum + (t.points * t.cap), 0);
  };
  ...
  return (
    <div className="min-h-screen bg-[#070713] text-white overflow-hidden">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-8 md:py-12">
          ...
          <button onClick={() => setShowLeaderboardModal(true)} className="glass-card p-6 hover-lift opacity-0 animate-fade-in-up text-left cursor-pointer group transition-all duration-300 hover:border-[#b9f0d7]/30" style={{ animationDelay: '450ms', animationFillMode: 'forwards' }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#b9f0d7]/20 to-[#b9f0d7]/5 border border-[#b9f0d7]/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                <svg className="w-6 h-6 text-[#b9f0d7]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 4v12l-4-2-4 2V4M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#0d0d1a] border border-[#7d85d0]/20">
                  <span className="text-[#b6bbff]/70 text-sm font-mono">{referralStats?.referral_code || '...'}</span>
                </div>
                <button onClick={copyReferralLink} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#ffc107] to-[#ffab00] text-[#070713] font-semibold rounded-xl hover:shadow-[0_0_20px_rgba(255,193,7,0.3)] transition-all duration-300">
                ...
                <button onClick={() => setShowReferralModal(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl glass-card border border-[#7d85d0]/20 hover:border-[#ffc107]/30 text-[#b6bbff]/70 hover:text-white transition-all">
            ...
            {nextRank && (
              <div>
                ...
                <div className="h-2 bg-[#0d0d1a] rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-[#6265fe] to-[#7d85d0] transition-all duration-500" style={{ width: `${rankProgress}%` }} />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="mb-6 flex items-center justify-between opacity-0 animate-fade-in-up" style={{ animationDelay: '600ms', animationFillMode: 'forwards' }}>
          ...
          <div className="flex items-center gap-3 text-xs">
            <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-[#b9f0d7]/10 text-[#b9f0d7]"><span className="w-2 h-2 rounded-full bg-[#b9f0d7]" />Active</span>
        ...
        <div className="space-y-4" ref={containerRef}>
          {CATEGORIES.map((category, catIndex) => {
            ...
            const hasActiveTasks = catTasks.some(t => t.status === 'active');
            const isLocked = !hasActiveTasks;
            const maxCp = categoryMaxCp(category.id) || category.max_cp;

            return (
              <div key={category.id} className="glass-card overflow-hidden opacity-0 animate-fade-in-up" style={{ animationDelay: `${650 + catIndex * 50}ms`, animationFillMode: 'forwards' }}>
                <button onClick={() => toggleCategory(category.id)} className="w-full p-5 flex items-center justify-between hover:bg-[#6265fe]/5 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isLocked ? 'bg-[#7d85d0]/10 border border-[#7d85d0]/20' : 'bg-gradient-to-br from-[#6265fe]/20 to-[#6265fe]/5 border border-[#6265fe]/20'}`}>
                      <CategoryIcon categoryId={category.id} isLocked={isLocked} />
                    </div>
                    <div className="text-left">
                      <div className="flex items-center gap-2">
                        <h3 className={`font-semibold ${isLocked ? 'text-[#7d85d0]/70' : 'text-white'}`}>{category.name}</h3>
                        {isLocked && (
                      ...
                      <p className={`text-sm ${isLocked ? 'text-[#7d85d0]/40' : 'text-[#b6bbff]/50'}`}>{category.description}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
      ...
      {/* Task Modal */}
      {selectedTask && (
        <TaskModal task={selectedTask} walletAddress={address || ''} referralStats={referralStats} onClose={() => setSelectedTask(null)} onSubmitted={handleTaskSubmitted} />
      )}

      {/* Leaderboard Modal */}
      {showLeaderboardModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowLeaderboardModal(false)} />
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {userLeaderboardEntry && userLeaderboardEntry.rank > 10 && (
                        <>
                          <tr className="bg-[#6265fe]/15 border-b-2 border-[#6265fe]/30">
                            <td className="py-4 px-6"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#6265fe] to-[#7d85d0] flex items-center justify-center text-sm font-bold text-white">{userLeaderboardEntry.rank}</div></td>
                            <td className="py-4 px-6"><div className="flex items-center gap-2"><span className="font-mono text-sm text-[#6265fe] font-semibold">{userLeaderboardEntry.wallet_address.slice(0, 6)}...{userLeaderboardEntry.wallet_address.slice(-4)}</span><span className="text-[10px] px-2 py-0.5 rounded-full bg-[#6265fe]/30 text-[#6265fe] font-bold">YOU</span></div></td>
                            <td className="py-4 px-6 text-right"><span className="font-bold text-[#6265fe]">{userLeaderboardEntry.points.toLocaleString()}</span></td>
                          </tr>
                          <tr><td colSpan={3} className="py-2 px-6 text-center text-[#7d85d0]/40 text-xs">...</td></tr>
                        </>
                      )}
                      {leaderboard.map((entry) => {
                        ...
                        return (
                          <tr key={entry.wallet_address} className={`border-b border-[#7d85d0]/5 transition-all duration-200 ${isCurrentUser ? 'bg-[#6265fe]/15' : 'hover:bg-[#6265fe]/5'}`}>
                            <td className="py-4 px-6">
                              {entry.rank <= 3 ? (
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${entry.rank === 1 ? 'bg-gradient-to-br from-[#b9f0d7] to-[#6265fe] text-[#070713]' : entry.rank === 2 ? 'bg-gradient-to-br from-[#c9e8ff] to-[#7d85d0] text-[#070713]' : 'bg-gradient-to-br from-[#7d85d0] to-[#6265fe] text-white'}`}>{entry.rank}</div>
                              ) : (
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${isCurrentUser ? 'bg-gradient-to-br from-[#6265fe] to-[#7d85d0] text-white' : 'bg-[#7d85d0]/10 text-[#7d85d0]'}`}>{entry.rank}</div>
                              )}
                            </td>
                            <td className="py-4 px-6"><div className="flex items-center gap-2"><span className={`font-mono text-sm ${isCurrentUser ? 'text-[#6265fe] font-semibold' : 'text-[#c9e8ff]'}`}>{entry.wallet_address.slice(0, 6)}...{entry.wallet_address.slice(-4)}</span>{isCurrentUser && <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#6265fe]/30 text-[#6265fe] font-bold">YOU</span>}</div></td>
                            <td className="py-4 px-6 text-right"><span className={`font-bold ${isCurrentUser ? 'text-[#6265fe]' : isTop3 ? 'text-[#b9f0d7]' : 'text-white'}`}>{entry.points.toLocaleString()}</span></td>
                          </tr>
                        );
                      })}
                    </tbody>
      ...
      {showReferralModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowReferralModal(false)} />
          <div className="relative glass-card w-full max-w-md overflow-hidden rounded-2xl animate-scale-in">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between text-xs mb-2"><span className="text-[#b6bbff]/50">Progress</span><span className="text-[#ffc107]">{referralStats?.verified_referrals || 0} / {referralStats?.max_referrals || 5}</span></div>
                <div className="h-2 bg-[#0d0d1a] rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-[#ffc107] to-[#ffab00] transition-all duration-500" style={{ width: `${((referralStats?.verified_referrals || 0) / (referralStats?.max_referrals || 5)) * 100}%` }} /></div>
              </div>
